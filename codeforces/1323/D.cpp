...
signed main() {
	...
	rep(k, 0, B) {
		mask |= 1 << k;
		vi b(a); trav(x, b) x &= mask;
		sort(all(b));
		int cnt = 0;
		rep(i, 0, n) {
			auto down = lb(all(b), (1 << k) - b[i]) - begin(b);
			auto up = ub(all(b), (1 << (k + 1)) - b[i] - 1) - begin(b);
			cnt += up - down;
			if (down <= i && i < up) --cnt;
			down = lb(all(b), (1 << (k + 1)) + (1 << k) - b[i]) - begin(b);
			up = ub(all(b), (1 << (k + 2)) - b[i] - 2) - begin(b);
			cnt += up - down;
			if (down <= i && i < up) --cnt;
		}
		assert(cnt % 2 == 0);
