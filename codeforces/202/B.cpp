...
bool subsq(const vi &a, const vi &b) {
    int i = 0, j = 0;
    while (j < sz(b)) {
...
signed main() {
    ...
    rep(i, 0, m) {
        ...
        int d = n * (n - 1) / 2 + 1, mx = 0;
        do {
            if (subsq(arc, prob))
                ckmax(mx, d - invcnt(prob));
        } while(next_permutation(all(prob)));
        if (ckmax(p, mx)) ans = i + 1;
    }
    
    if (ans == 0) pr("Brand new problem!");
    else {
        pr(ans, '\n');
        pr("[:"); rep(i, 0, p) pr('|'); pr(":]");
    }
