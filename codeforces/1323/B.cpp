...
signed main() {
	int n[2], k; re(n[0], n[1], k);
	vi nxt[2], a[2]; a[0] = vi(n[0]); a[1] = vi(n[1]);
	re(a[0], a[1]);
	rep(b, 0, 2) {
		nxt[b] = vi(n[b]);
		if (a[b][n[b] - 1] == 1) nxt[b][n[b] - 1] = n[b] - 1;
		else nxt[b][n[b] - 1] = n[b] - 2;
		per(i, 0, n[b] - 1)
			if (a[b][i] == 0) nxt[b][i] = i - 1;
