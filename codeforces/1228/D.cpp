...
vi solve() {
	...
	int r = 0;
	rep(u, 0, n)
		if (!binary_search(all(g[r]), u))
			v[0].pb(u);
	r = n;
	rep(u, 0, n) {
		if (!binary_search(all(v[0]), u)) {
			r = u;
			break;
		}
	}
	if (r == n) return {-1};
	rep(u, 0, n)
		if (!binary_search(all(g[r]), u) && !binary_search(all(v[0]), u))
			v[1].pb(u);
	
	r = n;
	rep(u, 0, n) {
		if (!binary_search(all(v[0]), u) && !binary_search(all(v[1]), u)) {
			r = u;
			break;
		}
	}
	...
	rep(u, 0, n)
		if (!binary_search(all(g[r]), u) && !binary_search(all(v[0]), u) && !binary_search(all(v[1]), u))
			v[2].pb(u);

	if (n != sz(v[0]) + sz(v[1]) + sz(v[2])) return {-1};
	rep(i, 0, 3) rep(j, i + 1, 3) m -= 1LL * sz(v[i]) * sz(v[j]);
	if (m != 0) return {-1};
	...
	rep(i, 0, 3)
		rep(j, i + 1, 3)
			trav(u, v[i])
				trav(w, v[j])
					if (!binary_search(all(g[u]), w) || !binary_search(all(g[w]), u))
						return {-1};
	vi res(n);
	rep(i, 0, 3) trav(u, v[i]) res[u] = i + 1;
	return res;
}
