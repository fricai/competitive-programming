...
namespace algebra {
	...
	public:
		...
		bool operator != (const modular &t) const { return r != t.r; }
		explicit operator int32_t() const { return r; }
...
struct end_t {
	ll t; bool b;
	end_t(ll f = 0, bool c = 0) : t{f}, b{c} {}
	bool operator<(const end_t& a) {
		if (a.t == t) return b < a.b;
		return t < a.t; 
	}
};
...
signed main() {
	...
	trav(e, s) {
		...
		if (st.empty() || x < (e.t - st.top()) * y)
			ans += x;
		else ans += (e.t - st.top()) * y, st.pop();
	}
	pr(ans);
}