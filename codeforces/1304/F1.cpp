...
const int N = 1 << 6, M = 1 << 15, K = 1 << 5;

int g[N][M];
ll p[N][M], dp[N][M], mp[N][M], ms[N][M];

signed main() {
	int n, m, k; re(n, m, k);
	rep(i, 0, n) rep(j, 0, m) re(g[i][j]);
	rep(i, 0, n) rep(j, 0, m) p[i][j + 1] = p[i][j] + g[i][j];
	...
	per(i, 0, n) {
		rep(j, 0, m - k + 1) {
			rep(l, 0, k)
				if (j + l <= m - k)
					ckmax(dp[i][j],
						dp[i + 1][j + l] + p[i][j + k] - p[i][j]
						+ p[i + 1][j + l] - p[i + 1][j]);
			rep(l, 0, k)
				if (0 <= j - l)
					ckmax(dp[i][j],
						dp[i + 1][j - l] + p[i][j + k] - p[i][j]
						+ p[i + 1][j + k] - p[i + 1][j + k - l]);
			if (j + k <= m - k)
				ckmax(dp[i][j],
					ms[i + 1][j + k] + p[i][j + k] - p[i][j]
					+ p[i + 1][j + k] - p[i + 1][j]);
			if (0 <= j - k)
				ckmax(dp[i][j],
				mp[i + 1][j - k] + p[i][j + k] - p[i][j]
				+ p[i + 1][j + k] - p[i + 1][j]);
		}

		mp[i][0] = dp[i][0];
		rep(j, 1, m - k + 1) mp[i][j] = max(mp[i][j - 1], dp[i][j]);
		per(j, 0, m - k + 1) ms[i][j] = max(ms[i][j + 1], dp[i][j]);
	}
