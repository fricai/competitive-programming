...
constexpr int Z = 1 << 5, B = 11;

map<int, int> m = {{2, 0}, {3, 1}, {5, 2}, {7, 3}, {11, 4}, {13, 5}, {17, 6}, {19, 7}, {23, 8}, {29, 9}, {31, 10}};
...
void solve() {
	...
	rep(S, 0, 1 << B) {
		...
		bool flag = false;
		rep(x, 0, n) if (!c[x]) flag = true;
		if (flag) continue;
		assert(cnt == __builtin_popcount(S));
		cout << cnt << '\n';
		rep(i, 0, n) cout << c[i] << ' '; cout << '\n';
		// rep(i, 0, n)
		// 	rep(j, 0, n)
		// 		if (c[i] == c[j])
		// 			assert(__gcd(a[i], a[j]) > 1);
		return; 
	}	
	assert(false);
}
