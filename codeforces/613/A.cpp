...
void re(point &p) { re(p.x, p.y); }
void pr(const point &p) { pr(make_pair(p.x, p.y)); }

ll cross(const point &p, const point &q) { // p x q = a k, gives a
...
ld segdist(const point &s, const point &e, const point &p) {
	if (s == e) return abs(s - p);
	ll d = norm(e - s), t = dot(p - s, e - s);
	if (t < 0)
		return abs(s - p);
	if (t > d)
		return abs(e - p);
	return abs(cross(s - p, e - p)) / sqrt(d);
}

signed main() {
	int n; point p; re(n, p);
	vector<point> q(n); re(q);
	ll mx = 0; ld mi = inf;
	trav(r, q) ckmax(mx, norm(r - p));
	rep(i, 0, n)
		ckmin(mi, segdist(q[i], q[(i + 1) % n], p));
	pr(pi * (mx - mi * mi));
}