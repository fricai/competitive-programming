...
void solve() {
	int n; string og; re(n, og);
	string t = og; int ans = 1;
	rep(k, 1, n) {
		string s = og;
		rotate(begin(s), begin(s) + k, end(s));
		if ((n - k) & 1) reverse(end(s) - k, end(s));
		if (ckmin(t, s)) ans = k + 1;
	}
	pr(t, '\n', ans, '\n');
}
...
signed main() {
	...
	// 	vi v(n);
	// 	rep(i, 0, n) v[i] = i;
	// 	rep(i, 0, n - k) reverse(begin(v) + i, begin(v) + i + k + 1);
	// 	vi w(n);
	// 	rep(i, 0, n) w[i] = i;
	// 	rotate(begin(w), begin(w) + k, end(w));
	// 	if ((n - k) & 1) reverse(end(w) - k, end(w));
	// 	assert(v == w);
	// }
	int t; re(t);
	while (t--) solve();
