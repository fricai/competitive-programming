...
signed main() {
	...
	const mint mapper[2] = {-1, +1};
	const mint inv2 = mint(1) / 2;

	vector<mint> B(sz(b));
	rep(i, 0, sz(b)) B[i] = mapper[b[i] - '0'];
	
	vector cnt(sz(a) / L, vector(max(0, sz(b) - L + 1), 0));
	for (int i = 0, x = 0; x + L <= sz(a); x += L, ++i) {
		vector<mint> A(L);
		rep(s, 0, L) A[L - s - 1] = mapper[a[s + x] - '0'];
