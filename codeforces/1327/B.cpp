...
void solve() {
	...
	rep(i, 0, n) g[i].clear();
	rep(i, 0, n) paired[i] = false;
	rep(i, 0, n) gp[i] = false;

	rep(i, 0, n) {
		int k; cin >> k; g[i].resize(k);
		rep(j, 0, k) cin >> g[i][j], --g[i][j];
	}
	rep(u, 0, n) {
		trav(v, g[u]) {
			if (!paired[v]) {
	...
	int bempty = -1, gempty = -1;
	rep(i, 0, n) if (!paired[i]) bempty = i + 1;
	rep(i, 0, n) if (!gp[i]) gempty = i + 1;
	if (bempty == -1 || gempty == -1) return void(cout << "OPTIMAL\n");
	cout << "IMPROVE\n";
	cout << gempty << ' ' << bempty << '\n';
}
