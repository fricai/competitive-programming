...
string solve() {
	string s; re(s); int n = sz(s);
	rep(c, 0, 26) a[c].clear();
	rep(c, 0, 26) vis[c] = false;
	rep(c, 0, 26) cmp[c].clear();
	...
	rep(i, 0, c) {
		...
		int e = 0;
		trav(u, cmp[i]) e += sz(a[u]);
		assert(e % 2 == 0);
		if (e / 2 != sz(cmp[i]) - 1) return "";
		if (ends != 2) return "";
		
		while (l != r) {
			vis[l] = true;
			res += string(1, l + 'a');
			if (vis[*a[l].begin()]) l = *a[l].rbegin();
			else l = *a[l].begin();
		}
		res += string(1, r + 'a');
	}
