...
ll abs(const point& a, const point& b) {
    return abs(a.x - b.x) + abs(a.y - b.y);
}
ll abs(const point& a) {
...
signed main() {
    ...
    vector<point> p = {o};
    ll inf = 1e17;
    while (abs(p.back()) < inf) p.pb(next(p.back())); 
    int B = sz(p);
    rep(z, 0, B)
        rep(l, 0, z + 1)
            rep(r, z, B)
                if (abs(s, p[z]) + abs(p[l], p[r]) + min(abs(p[z], p[l]), abs(p[z], p[r])) <= t)
                    ckmax(k, r - l + 1);
    pr(k);
}